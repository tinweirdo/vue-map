...
function bind() {
    $("#btn_Search").on("click", function () {
        ...
        start_time = new Date(start_timestramp).toLocaleString('chinese');
        end_time = new Date(end_timestramp).toLocaleString('chinese');
        query_where = `("globalId"='${globalId}') and (last_edited_date >= timestamp '${start_time}' and last_edited_date<timestamp '${end_time}' )`;
        StreamServer({
            where: query_where,
            // returnGeometry: false,
            // outFields: 'last_edited_date,istFlow',
...
function StreamServer(param, cb) {
    $.getJSON(layerUrl + "/query", param, function (res) {
        ...
        lists = lists.filter(item => (item.z || item.zb));
    ...
    option = {
        ...
        tooltip: {
            ...
            formatter: function (val) {
                let label = "";
                val.forEach((ele, i) => {
                    label += `${ele.marker} ${name[i]}: ${ele.data} m<br/>`
                });
                return label;
            }
        },
